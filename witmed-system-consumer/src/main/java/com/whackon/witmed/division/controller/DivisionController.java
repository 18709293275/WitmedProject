package com.whackon.witmed.division.controller;
 ...
 * @since 1.0.0
 */
@Api(value = "系统功能 - 行政区划信息控制层类", tags = "系统功能 - 行政区划信息控制层类")
...
public class DivisionController extends BaseController {
    ...
    public ResponseVO queryPageByQuery(@PathVariable("pageNum") Integer pageNum, @PathVariable("pageSize") Integer pageSize, @RequestBody DivisionVO queryVO)
            throws Exception {
        // 判断用户是否已经登录
        ...
        if (adminVO != null) {
            ...
            // 进行分页查询
            PageVO<DivisionVO> pageVO = transport.getPageByQuery(queryPageVO);
    ...
    @PostMapping("/list")
    public ResponseVO queryListByQuery(@RequestBody DivisionVO queryVO) throws Exception {
        // 判断用户是否已经登录
        AdminVO adminVO = adminTransport.getByToken(request.getHeader(BaseConstants.TOKE_HEADER));
        if (adminVO != null) {
    ...
    @PostMapping("/insert")
    public ResponseVO insert(@RequestBody DivisionVO vo) throws Exception {
        // 判断用户是否登录
        AdminVO adminVO = adminTransport.getByToken(request.getHeader(BaseConstants.TOKE_HEADER));
        if (adminVO != null) {
            // 校验用户所填写信息是否有效
    ...
    public ResponseVO update(@RequestBody DivisionVO vo) throws Exception {
        ...
        if (adminVO != null) {
            ...
            if (StrUtil.isBlank(vo.getName()) && StrUtil.isBlank(vo.getCode())) {
                // 进行唯编码一性校验
                DivisionVO checkVO = transport.getByCode(vo.getCode());
